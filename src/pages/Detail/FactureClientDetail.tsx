...
const FactureClientDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      try {
        const res = await api.get(`/factures-client/${id}`)
        setDoc(res.data)
      } catch (e: any) {
        setError(e.response?.data?.error || 'Erreur chargement facture client')
      } finally {
        setLoading(false)
      }
    }
    ...
    <DashboardLayout>
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Box>
            <Typography variant="h5" fontWeight="bold">Facture Client {doc.numero}</Typography>
            <Typography variant="body2" color="text.secondary">Client: {doc.client?.name} ({doc.client?.code})</Typography>
          </Box>
          <Box>
              ...
              <Table>
                ...
                <TableBody>
                  {doc.lignes.map((l: any) => {
                    const designationLines = l.designation?.split('\n') || [l.designation || ''];
                    return (
                      <TableRow key={l.id}>
                        <TableCell>
                          <div>{designationLines[0]}</div>
                          {designationLines.slice(1).map((line: string, i: number) => (
                            <div key={i} style={{ fontSize: '0.85em', color: '#666', marginTop: '2px' }}>{line}</div>
                          ))}
                        </TableCell>
...
export default FactureClientDetail