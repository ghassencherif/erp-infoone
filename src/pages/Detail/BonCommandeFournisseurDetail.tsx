...
const BonCommandeFournisseurDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      try {
        ...
        setError(e.response?.data?.error || 'Erreur chargement bon de commande fournisseur')
      } finally {
        setLoading(false)
      }
    }
    ...
    <DashboardLayout>
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Box>
            <Typography variant="h5" fontWeight="bold">Bon de Commande Fournisseur {doc.numero}</Typography>
...
export default BonCommandeFournisseurDetail