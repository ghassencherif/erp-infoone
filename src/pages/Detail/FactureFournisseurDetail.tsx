...
const FactureFournisseurDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoc = async () => {
      try {
        const res = await api.get(`/factures-fournisseur/${id}`)
        setDoc(res.data)
      } catch (e: any) {
        setError(e.response?.data?.error || 'Erreur chargement facture fournisseur')
      } finally {
        setLoading(false)
      }
    }
    ...
    <DashboardLayout>
      <Box sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Box>
            <Typography variant="h5" fontWeight="bold">Facture Fournisseur {doc.numero}</Typography>
            <Typography variant="body2" color="text.secondary">Fournisseur: {doc.fournisseur?.name}</Typography>
          </Box>
          <Box>
